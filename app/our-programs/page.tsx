...
const page = () => {
	...
	const getPrograms = async () => {
		const programResponse = await fetch(`${process.env.STRAPI_URL}/api/our-programs?populate=*`);
	...
	const confirmDelete = async (e) => {
		try {
			const response = await fetch("/api/delete_program", {
				method: 'DELETE',
				headers: {
			...
			if (response.ok){
				setData(data.filter((rec) => rec.documentId != deleteId));
				setDeleteId("");
				setDeleteFileId("");
				setDeleteImageId("");
				setShowModal(false);
  ...
  return (
    <div className="flex flex-col justify-center items-center md:ml-[220px] p-8 gap-8">
      ...
      <section className="flex justify-between items-center w-full">
				...
				<a href="/our-programs/add" 
				className="px-5 py-3 rounded-lg bg-[#b3ff00] hover:bg-[#9ee004] 
				active:ring-offset-2 active:ring-2 active:ring-neutral-800 text-center font-semibold">Add a program</a>
			</section>
			{data.length == 0 && 
			...
			{(data[0] != 0 && data.length > 0) &&
				...
				{data.map((rec) => (
					<div className="relative flex flex-row justify-start items-center w-full h-max bg-[#242628] p-3 pr-0 space-x-3"
					key={rec.documentId}>
						...
						<div className="text-white basis-[70%] text-wrap">
							...
							<p className="text-justify min-[320px]:max-sm:static sm:hidden">{rec.Level}</p>
							<p className="text-justify min-[320px]:max-sm:hidden">{rec.Description}</p>
						</div>
						...
						<div className="flex flex-col flex-shrink-0 items-center justify-around w-[120px] h-full right-0 space-y-5">
							...
							<button type="button" value={`["${rec.documentId}", ${rec.Thumbnail.id}, ${rec.Document.id}]`} onClick={onDelete}
