...
import { Category, StrapiFile } from "../../../Types";
...
const Page = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const [selectedExistedFile, setSelectedExistedFile] = useState<number | null>(
    null
  ); // Selected existed file
  const [selectedExistedFileName, setSelectedExistedFileName] =
    useState<string>("Select Existed File"); // Selected existed file name
  ...
  const [selectedExistedImage, setSelectedExistedImage] = useState<
    number | null
  >(null); // Selected existed image
  const [selectedExistedImageName, setSelectedExistedImageName] =
    useState<string>("Select Existed Image"); // Selected existed image name

  const [showFilesPickWindow, setShowFilesPickWindow] =
    useState<boolean>(false);
  const [showImagesPickWindow, setShowImagesPickWindow] =
    useState<boolean>(false);
  ...
  const getProgramById = async () => {
    const response = await fetch(
      `${process.env.STRAPI_URL}/api/our-programs?populate=*&filters[documentId][$eq]=${id}`
    );
  ...
  const getFilesImages = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.STRAPI_URL}/api/upload/files`
      );
      const res_json: (StrapiFile[]) = await response.json();
      setExistedFiles(
        res_json.filter(
          (rec) => rec.ext.includes("pdf") || rec.ext.includes("PDF")
        )
      );
      setExistedImages(
        res_json.filter(
          (rec) =>
            !rec.ext.includes("pdf") &&
            !rec.ext.includes("PDF") &&
  ...
  const getCategories = async () => {
    const response = await fetch(
      `${process.env.STRAPI_URL}/api/categories?sort=Category`
    );
    const res_json = await response.json();
      ...
      {showFilesPickWindow && (
        <BlockBackground bgColor="bg-neutral-500/[0.5]">
          <FileCards
            fileType="pdf"
            files={existedFiles}
            onFileSelected={changeSelectedExistedFile}
            onClose={() => setShowFilesPickWindow(false)}
          />
        </BlockBackground>
      )}
      ...
      {showImagesPickWindow && (
        <BlockBackground bgColor="bg-neutral-500/[0.5]">
          <FileCards
            fileType="image"
            files={existedImages}
            onImageSelected={changeSelectedExistedImage}
            onClose={() => setShowImagesPickWindow(false)}
          />
        </BlockBackground>
      )}
          ...
          <div className="flex flex-col space-y-3 basis-[100%] lg:basis-[45%]">
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex flex-row gap-2 w-full">
                ...
                <div
                  className={`py-3 px-1 flex justify-center items-center cursor-pointer w-[20%] rounded-md
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex flex-row gap-2 w-full">
                {uploadFile && (
                  ...
                  >
                    <p className="absolute max-w-[80%] text-ellipsis text-nowrap overflow-clip">
                      {fileName}
                    </p>
                    <input
                      type="file"
                ...
                {!uploadFile && (
                  <div
                    className="relative w-[80%] bg-neutral-200 border-neutral-200 border-2 h-[52px]
									cursor-pointer py-3 px-1 rounded-md text-black"
                    onClick={handleShowFilesPickWindow}
                  >
                    <p className="absolute max-w-[80%] text-ellipsis text-nowrap overflow-clip">
                      {selectedExistedFileName}
                    </p>
                  </div>
                )}
                <div
              ...
              <div className="flex flex-row gap-2 w-full">
                ...
                {!uploadImage && (
                  <div
                    className="relative w-[80%] bg-neutral-200 border-neutral-200 border-2 h-[52px]
									cursor-pointer py-3 px-1 rounded-md text-black"
                    onClick={handleShowImagesPickWindow}
                  >
                    <p className="absolute max-w-[80%] text-ellipsis text-nowrap overflow-clip">
                      {selectedExistedImageName}
                    </p>
                  </div>
                )}
                <div
