				...
				<div className="flex justify-center flex-wrap gap-3 md:gap-5 max-h-full overflow-auto p-5">
					{files.map((file) => (
						<div className="basis-[18%] flex flex-col justify-center items-center" key={file.id}>
							...
							<div className="flex justify-center items-center w-full border-2 border-neutral-500 border-t-0 rounded-b-md mb-1">
								<p className="max-w-[80%] text-ellipsis text-nowrap overflow-clip">{file.name}</p>
							</div>
							
							<div value={`[${file.id}, "${file.name}"]`} onClick={onImageSelected}
								className="bg-[#b3ff00] text-green-800 hover:bg-[#9ee004] w-max p-1 cursor-pointer
					...
					{files.map((file) => (
						<div className="flex flex-row items-center gap-1 w-full" key={file.id}>
							<div className="relative w-[80px] h-[100px] flex justify-center">
								...
								<a href={`${process.env.STRAPI_URL}${file.url}`} target="_blank"
									className="absolute w-[80px] h-[100px] text-white 
									flex justify-center items-center 
									transition-all linear duration-300 
									opacity-0 hover:bg-neutral-800/[0.8] hover:opacity-100 z-[100]">
									View
							...
							<div className="w-full flex flex-col justify-start">
								<p className="max-w-[70%] text-ellipsis text-nowrap overflow-clip">{file.name}</p>
								<div value={`[${file.id}, "${file.name}"]`} onClick={onFileSelected}
									className="bg-[#b3ff00] text-green-800 hover:bg-[#9ee004] w-max p-1 cursor-pointer
									active:ring-offset-2 active:ring-2 active:ring-neutral-800 font-semibold rounded-md"
								>
									Select
